...
import { PlusCircle, Trash2, Edit, Route as RouteIcon } from "lucide-react";
import Link from "next/link";

// Mock data - replace with API call
const mockBusRoutes = [
  { id: "R1", name: "Main Campus Ring", stops: 5, frequency: "15 min", status: "Active" },
  { id: "R2", name: "East Dorms - Library", stops: 3, frequency: "20 min", status: "Active" },
  { id: "R3", name: "West Gate - Sports Complex", stops: 4, frequency: "30 min", status: "Inactive" },
  { id: "R4", name: "City Center Express", stops: 2, frequency: "1 hour", status: "Active" },
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Manage Bus Routes</h1>
          <p className="text-muted-foreground">Add, edit, or remove ring bus routes.</p>
        </div>
        <Button>
      ...
      <Card>
        ...
        <CardContent>
          {mockBusRoutes.length > 0 ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <h3 className="font-medium text-lg flex items-center">
                        <RouteIcon className={`mr-2 h-5 w-5 ${route.status === 'Active' ? 'text-green-500' : 'text-red-500'}`} /> 
                        {route.name}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Stops: {route.stops} | Frequency: {route.frequency} | Status: <span className={route.status === 'Active' ? 'text-green-600' : 'text-red-600'}>{route.status}</span>
                      </p>
                    </div>
            ...
            <p className="text-muted-foreground">No bus routes found. Add some to get started.</p>
          )}
        </CardContent>
      </Card>
