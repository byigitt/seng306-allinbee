...
import { Trash2, Star, PlusCircle } from "lucide-react";
...
export default function FavoriteRoutesPage() {
  const [favoriteRoutes, setFavoriteRoutes] = useState<FavoriteRoute[]>(initialFavoriteRoutes);
  const [selectedRouteToAdd, setSelectedRouteToAdd] = useState<string>("");

  const addFavoriteRoute = () => {
    if (selectedRouteToAdd && !favoriteRoutes.find(fr => fr.id === selectedRouteToAdd)) {
      const routeToAdd = allMockRoutes.find(r => r.id === selectedRouteToAdd);
      if (routeToAdd) {
        setFavoriteRoutes([...favoriteRoutes, { id: routeToAdd.id, name: routeToAdd.name }]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Favorite Bus Routes</h1>
        <p className="text-muted-foreground">Manage your preferred ring bus routes for quick access.</p>
      </div>
        ...
        <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div className="grid flex-1 gap-2">
                ...
                <Select value={selectedRouteToAdd} onValueChange={setSelectedRouteToAdd}>
                    ...
                    <SelectContent>
                        {allMockRoutes.filter(r => !favoriteRoutes.some(fr => fr.id === r.id)).map(route => (
                            <SelectItem key={route.id} value={route.id}>{route.name}</SelectItem>
                        ))}
                         {allMockRoutes.filter(r => !favoriteRoutes.some(fr => fr.id === r.id)).length === 0 && (
                            <p className="p-2 text-sm text-muted-foreground">All routes added.</p>
                        )}
                    </SelectContent>
                </Select>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          {favoriteRoutes.length === 0 && <CardDescription>You haven&apos;t added any favorite routes yet.</CardDescription>}
        </CardHeader>
          ...
          {favoriteRoutes.map((route) => (
            <div key={route.id} className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                {route.customName && <p className="text-sm text-muted-foreground">({route.name})</p>}
              </div>
              {/* TODO: Add functionality to edit customName */}
              <Button variant="ghost" size="icon" onClick={() => removeFavoriteRoute(route.id)}>
